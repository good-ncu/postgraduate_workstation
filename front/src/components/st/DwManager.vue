...
<!--        </el-col>-->
      ...
      <el-table :data="userList" border stripe style="font-size: 18px" id="user_table">
        ...
        <el-table-column label="高校联系人及电话" prop="user_gxphone"></el-table-column>
        <el-table-column label="报备单位联系人及电话" prop="user_sbdwphone"></el-table-column>
  ...
  methods: {
    ...
    async getExcel() {
      ...
      require.ensure([], () => {
        ...
        // const json = arrData;
        const  tHeader = ['用户系统编号','高校名称','院校名称','报备单位名称','账号名','账户密码','高校联系人及电话','报备单位联系人及电话'];
        const filterVal = ['user_id', 'univ_name','univ_code','user_sbdw', 'user_name','user_passwd','user_gxphone','user_sbdwphone'];
        const json = arrData;

        const data = this.formatJson(filterVal, json);
        // console.log(data, "data");
        export_json_to_excel(tHeader, data, `系统报备单位账号信息表`);
      });
    },
    //保持输出顺序与表头一致
    ...
    // 获取所有用户
    async getUserList() {
      const {data: res} = await this.$http.get(`allUser_dw?query=${this.queryInfo.query}&pageNum=${this.queryInfo.pageNum}&pageSize=${this.queryInfo.pageSize}`);
    ...
    //   let result = "";
    //   while (flag == "0") {
    //     rdNum = Math.random().toFixed(6).slice(-3);
    //     result = date.getFullYear() + "gzz" + rdNum;
    //     console.log(result);
    ...
    //
    //   //生成院校代码加5位随机数的十位数随机密码
    //   let passNum = univ_code + Math.random().toFixed(6).slice(-5)
