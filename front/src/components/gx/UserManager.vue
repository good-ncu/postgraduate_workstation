<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>报备账号管理</el-breadcrumb-item>
    </el-breadcrumb>
    ...
    <el-card>
      ...
      <el-table :data="userList" border stripe style="font-size: 18px" id="user_table">
        ...
        <el-table-column label="报备单位名称" prop="user_sbdw"></el-table-column>
        <el-table-column label="高校联系人电话" prop="user_gxphone"></el-table-column>
        <el-table-column label="报备单位联系人电话" prop="user_sbdwphone"></el-table-column>
        <el-table-column label="账号状态" prop="user_status" width="120px" align="center">
          <!--作用域插槽-->
        ...
        <el-table-column label="修改用户信息" width="150px" align="center">
          <template slot-scope="scope">
            <!--修改用户信息-->
            <el-tooltip effect="dark" content="修改申报单位信息" placement="top-start" :enterable="false">
              <el-button type="primary" icon="el-icon-edit" size="Medium"
                         @click="showEditDialog(scope.row.user_id)"></el-button>
            </el-tooltip>
            <!--修改用户密码-->
            <el-tooltip effect="dark" content="修改用户密码" placement="top-start" :enterable="false">
              <el-button type="warning" icon="el-icon-setting" size="Medium"
                         @click="passwdEditDialog(scope.row.user_id)"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="添加用户" :visible.sync="addDialogVisible" width="50%" @close="addDialogClosed">
      ...
      <el-form :model="addForm" :rules="addFormRules" ref="addFormRef" label-width="70px" id="add_user_form">
        <el-form-item label="工作站名称" prop="user_sbdw" label-width="230px">
          <el-input v-model="addForm.user_sbdw" class="user_add_input"></el-input>
        </el-form-item>
        ...
        <el-form-item label="高校联系人及电话" prop="user_gxphone" label-width="230px">
          <el-input v-model="addForm.user_gxphone" class="user_add_input"></el-input>
        </el-form-item>
        <el-form-item label="申报单位联系人及电话" prop="user_sbdwphone" label-width="230px">
          <el-input v-model="addForm.user_sbdwphone" class="user_add_input"></el-input>
        </el-form-item>
      </el-form>
      <!--内容底部区域-->
  ...
  methods: {
    ...
    async getExcel() {
      ...
      require.ensure([], () => {
        ...
        // const json = arrData;
        const tHeader = ['用户系统编号', '报备单位名称', '账号名', '账户密码', '高校联系人及电话', '报备单位联系人及电话'];
        const filterVal = ['user_id', 'user_sbdw', 'user_name', 'user_passwd', 'user_gxphone', 'user_sbdwphone'];
        const json = arrData;

        const data = this.formatJson(filterVal, json);
        // console.log(data, "data");
        export_json_to_excel(tHeader, data, `${this.self_info.univ_name}报备单位账号信息表`);
      });
    },
    //保持输出顺序与表头一致
    ...
    addUser() {
      this.$confirm('确认添加用户信息？', '提示', {
        ...
        this.$refs.addFormRef.validate(async valid => {
          ...
          const{data: res0} = await this.$http.put(`addUnivIndex?univ_code=${this.self_info.univ_code}`);
          if(res0 != "success"){
          ...
          console.log(`报备单位：${this.addForm.user_sbdw}\n用户名：${this.addForm.user_name}\n密码：${this.addForm.user_passwd}`);
          this.downloadTxt(`报备单位：${this.addForm.user_sbdw}\n用户名：${this.addForm.user_name}\n密码：${this.addForm.user_passwd}`, `${this.self_info.univ_name}-${this.addForm.user_sbdw}账号文件`)
    ...
    // 获取所有用户
    async getUserList() {
      const {data: res} = await this.$http.get(`allUser?query=${this.queryInfo.query}&pageNum=${this.queryInfo.pageNum}&pageSize=${this.queryInfo.pageSize}&univ_code=${this.self_info.univ_code}`);
    ...
    async genUserNameAndPasswd() {
      let date = new Date();//当前时间
      let univ_code = this.self_info.univ_code;
      let flag = 0;
      let rdNum = "";
      let result = "";

      const {data: res1} = await this.$http.put(`getUnivIndex?univ_code=${univ_code}`);
      console.log(res1);
      let univ_index = res1["univ_index"];

      if(univ_index >0 && univ_index<9){
        univ_index = "00"+univ_index;
      }else if(univ_index >10 && univ_index<99){
        univ_index = "0"+univ_index;
      }

      result = date.getFullYear() + "gzz" + univ_code + univ_index;
      console.log(result);

      // const {data: res} = await this.$http.put(`searchUserName?user_name=${result}`);

      if (res1 == null || res1 == "error") {
        return this.$message.error("服务器异常，请联系管理员");
      }

      this.addForm.user_name = result;

      //生成院校代码加3位随机数的十位数随机密码
      let passNum = univ_code + Math.random().toFixed(6).slice(-3)
      this.addForm.user_passwd = passNum;

      this.addForm.univ_name = this.self_info.univ_name;
      this.addForm.univ_code = this.self_info.univ_code;
    },
