    ...
    <el-card>
      ...
      <table class="search_box">
        <tr style="height: 80px; text-align: right;border: 1px solid;font-size: 16px" align="center">
          ...
          <td align="left">
            <el-select v-model="queryInfo.query_year" clearable placeholder="请选择">
              <el-option
      ...
      <el-table :data="ApplyList" border stripe style="font-size: 16px" ref="multipleTable">
        ...
        <el-table-column label="提交年份" prop="apply_year" align="center"></el-table-column>
        <el-table-column label="最后一次操作时间" prop="apply_lastopt" align="center"></el-table-column>
  ...
  methods: {
    ...
    async getExcel() {
      ...
      require.ensure([], () => {
        ...
        // const json = arrData;
        const tHeader = ['申请编号', '申请年份', '最后一次操作时间', '申请单位名称', '申请单位性质', '技术领域', '统一社会信用代码', '联合培养学科或专业类别', '学科专业所在院系', '高校牵头导师', '所在市', '工作站类型', '审核状态码（0：未审核，1：已通过，2：已退回）'];
        const filterVal = ['apply_id', 'apply_year', 'apply_lastopt', 'apply_dwname', 'apply_dwxz', 'apply_jsly', 'apply_scode', 'apply_lhpydis', 'apply_disyx', 'apply_gxds', 'apply_city', 'apply_worklx', 'apply_status'];
        const json = arrData;

        const data = this.formatJson(filterVal, json);
        // console.log(data, "data");
        export_json_to_excel(tHeader, data, "历年报备查询数据表");
      });
    },
    //保持输出顺序与表头一致
    ...
    // 获取所有用户申请的列表
    async getApplyList() {
      const {data: res} = await this.$http.get(`getApply_gx_and_year?query_lhpy=${this.queryInfo.query_lhpy}&query_city=${this.queryInfo.query_city}&query_lx=${this.queryInfo.query_lx}&query_status=${this.queryInfo.query_status}&query_year=${this.queryInfo.query_year}&pageNum=${this.queryInfo.pageNum}&pageSize=${this.queryInfo.pageSize}&univ_code=${this.self_info.univ_code}`);
      // this.queryInfo.query_lhpy = '';
      // this.queryInfo.query_status = '';
      // this.queryInfo.query_lx = '';
      // this.queryInfo.query_city = '';
